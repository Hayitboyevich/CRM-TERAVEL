@php
    ...
    $viewFirstColumn = user()->permission('view_first_column');
    $viewUnsortedLead = user()->permission('view_unsorted_leads');
@endphp
@foreach ($result['boardColumns'] as $key => $column)
    @if($viewUnsortedLead && $column->priority == 1 || ($column->priority != 1))
            ...
            <div class="minimized rounded bg-additional-grey border-grey mr-3">
                ...
                <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
                    ...
                    <span class="b-p-badge bg-grey f-13 px-2 py-2 text-lightest font-weight-bold rounded d-inline-block"
                          id="lead-column-count-{{ $column->id }}">{{ $column->leads_count }}</span>
            ...
            <div class="board-panel rounded bg-additional-grey border-grey mr-3">
                ...
                <div class="mx-3 mt-3 mb-1 b-p-header">
                    <div class="d-flex">
                        ...
                        <span
                                class="b-p-badge bg-grey f-13 px-2 text-lightest font-weight-bold rounded d-inline-block"
                                id="lead-column-count-{{ $column->id }}">{{ $column->leads_count }}</span>
                    ...
                    <div
                            class="mr-3 ml-4 f-11 text-dark-grey">{{ currency_format($column->total_value, company()->currency_id) }}</div>
                </div>
                ...
                <div class="b-p-body">
                    ...
                    <div class="b-p-tasks" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}">
                        <div
                                class="card rounded bg-white border-grey b-shadow-4 m-1 mb-3 no-task-card move-disable {{ ($column->leads_count > 0 ) ? 'd-none' : '' }}">
                            <div class="card-body">
                        ...
                        @foreach ($column['leads'] as $lead)
                            <x-cards.lead-card
                                    :draggable="($changeStatusPermission == 'all') ? 'true' : 'false'"
                                    :lead="$lead"/>
...
<script>
    drake.on('drop', function (element, target, source, sibling) {
        ...
        const boardColumnId = $('#' + target.id).data('column-id');
        const movingTaskId = $('#' + element.id).data('task-id');
        const sourceBoardColumnId = $('#' + source.id).data('column-id');
        const sourceColumnCount = parseInt($('#lead-column-count-' + sourceBoardColumnId).text());
        const targetColumnCount = parseInt($('#lead-column-count-' + boardColumnId).text());
