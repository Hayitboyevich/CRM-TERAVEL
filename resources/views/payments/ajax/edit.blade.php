                ...
                <div class="row p-20">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="payment_project_id" :fieldLabel="__('app.project')"
                            ...
                            @foreach ($projects as $project)
                                <option @if ($project->id == $payment->project_id) selected
                                        @endif value="{{ $project->id }}" data-currency-id="{{ $project->currency_id }}"
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="payment_invoice_id" :fieldLabel="__('app.invoice')"
                            ...
                            @if ($payment->invoice_id)
                                <option selected data-currency-code="{{$payment->currency->currency_code}}"
                                        data-currency-id="{{ $payment->currency_id }}"
                                        value="{{ $payment->invoice_id }}">
                                    {{ $payment->invoice->invoice_number }}</option>
                            @endif
                            @foreach ($invoices as $item)
                                @if ($payment->invoice_id != $item->id)
                                    <option data-currency-id="{{ $item->currency->id }}"
                                            value="{{ $item->id }}">{{ $item->invoice_number }}</option>
                                @endif
                            @endforeach
                        </x-forms.select>
                    </div>
                    ...
                    <div class="col-lg-3 col-md-6">
                        ...
                        <x-forms.select fieldId="currency" :fieldLabel="__('app.currency')" fieldName="currency"
                            ...
                            @foreach ($currencies as $currency)
                                <option @if ($currency->id == $payment->currency_id) selected
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.text fieldId="exchange_rate" :fieldLabel="__('modules.currencySettings.exchangeRate')"
                                      ...
                                      :fieldValue="$payment->exchange_rate"
                                      :fieldReadOnly="($companyCurrency->id == $payment->currency_id)"
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="payment_gateway_id" :fieldLabel="__('modules.payments.paymentGateway')"
                                        ...
                                        search="true">
                            <option value="">--</option>
                            <option @if ($payment->gateway == 'Offline') selected
                                    @endif value="Offline">{{ __('modules.offlinePayment.offlinePayment') }}</option>
                            @if ($paymentGateway->paypal_status == 'active')
                                <option @if ($payment->gateway == 'paypal') selected
                                        @endif value="paypal">{{ __('app.paypal') }}</option>
                            @endif
                            @if ($paymentGateway->stripe_status == 'active')
                                <option @if ($payment->gateway == 'stripe') selected
                                        @endif value="stripe">{{ __('app.stripe') }}</option>
                            @endif
                            @if ($paymentGateway->razorpay_status == 'active')
                                <option @if ($payment->gateway == 'stripe') selected
                                        @endif @if ($payment->gateway == 'paypal') selected
                                        @endif value="razorpay">{{ __('app.razorpay') }}</option>
                            @endif
                            @if ($paymentGateway->paystack_status == 'active')
                                <option @if ($payment->gateway == 'paystack') selected
                                        @endif value="paystack">{{ __('app.paystack') }}</option>
                            @endif
                            @if ($paymentGateway->mollie_status == 'active')
                                <option @if ($payment->gateway == 'mollie') selected
                                        @endif value="mollie">{{ __('app.mollie') }}</option>
                            @endif
                            @if ($paymentGateway->payfast_status == 'active')
                                <option @if ($payment->gateway == 'payfast') selected
                                        @endif value="payfast">{{ __('app.payfast') }}</option>
                            @endif
                            @if ($paymentGateway->authorize_status == 'active')
                                <option @if ($payment->gateway == 'authorize') selected
                                        @endif value="authorize">{{ __('app.authorize') }}</option>
                            @endif
                            @if ($paymentGateway->square_status == 'active')
                                <option @if ($payment->gateway == 'square') selected
                                        @endif value="square">{{ __('app.square') }}</option>
                            @endif
                            @if ($paymentGateway->flutterwave_status == 'active')
                                <option @if ($payment->gateway == 'flutterwave') selected
                                        @endif value="flutterwave">{{ __('app.flutterwave') }}</option>
                            @endif
                        </x-forms.select>
                    </div>
                            ...
                            <x-forms.select fieldId="add_offline_methods"
                                ...
                                @foreach ($methods as $method)
                                    @if($method->status == 'yes')
                                        <option @if($payment->offline_method_id == $method->id) selected
                                                @endif value={{$method->id}}>{{$method->name}}</option>
                                    @endif
                                @endforeach
                            </x-forms.select>
                        </div>
                            ...
                            <x-forms.select fieldId="bank_account_id" :fieldLabel="__('app.menu.bankaccount')"
                                ...
                                @foreach ($bankDetails as $bankDetail)
                                    <option value="{{ $bankDetail->id }}"
                                            @if($bankDetail->id == $payment->bank_account_id) selected @endif>@if($bankDetail->type == 'bank')
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="status" :fieldLabel="__('app.status')" fieldName="status">
                            ...
                            <option
                                    data-content="<i class='fa fa-circle mr-1 text-red f-10'></i> {{ __('app.failed') }}"
                                    @if ($payment->status == 'failed') selected
                                    @endif
                                    value="failed">@lang('app.failed')</option>
                        </x-forms.select>
                    </div>
