...
@php
    $addPermission = user()->permission('edit_sms');
@endphp
                        ...
                        <div class="row p-20 pb-0">
                            <div class="col-lg-4 col-md-6">
                                <x-forms.select
                                        ...
                                        multiple="true"
                                        search="true">
                                    <option @if(in_array(0, json_decode($sms->user_id, true))) selected
                                            @endif value="0">Все
                                    </option>
                                    @foreach ($clients as $client)
                                        <option
                                                @if(in_array($client->id, json_decode($sms->user_id, true))) selected
                                                @endif
                                                value="{{ $client->id }}">{{ ($client->name . ' (' . $client->mobile.')') }}</option>
                                    @endforeach
                                </x-forms.select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group my-3">
                                    ...
                                    <div class="input-group">
                                        <input value="{{date('d-m-Y', strtotime($sms->delivery_date))}}"
                                               type="text" id="delivery_date" name="delivery_date"
                            ...
                            <div class="col-lg-3 col-md-6">
                                <div class="bootstrap-timepicker timepicker">
                                    <x-forms.text :fieldLabel="__('modules.events.startOnTime')"
                                                  ...
                                                  :fieldValue="date('H:i', strtotime($sms->delivery_date))"
                                                  fieldRequired="true"/>
                                </div>
                            </div>
                        </div>
