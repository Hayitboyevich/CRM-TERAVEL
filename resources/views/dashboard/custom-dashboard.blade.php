        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize"
                       for="operator">Оператор</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" name="operator" id="operator" data-live-search="true"
                                data-size="8">
                            <option value="all">@lang('app.all')</option>
                            @foreach ($operators as $operator)
                                <x-user-option :user="$operator"/>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize"
                       for="operator">Партнер</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" name="partner" id="partner" data-live-search="true"
                                data-size="8">
                            <option value="all">@lang('app.all')</option>
...
@section('content')
    <div class="content-wrapper">
        ...
        <div class="row mt-3">
            <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
                <a href="{{ route('applications.index') }}">
                    <x-cards.widget :title="__('app.clientDebits')"
                                    :value="currency_format($statistics->debt, currency_get('USD')->id)"
                                    icon="layer-group">
                    </x-cards.widget>
                </a>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
                <a href="{{ route('applications.index') }}">
                    <x-cards.widget :title="__('app.planProfit')"
                                    :value="currency_format($statistics->income, currency_get('USD')->id)"
                                    icon="layer-group">
                    </x-cards.widget>
                </a>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
                <a href="{{ route('applications.index') }}">
                    <x-cards.widget :title="__('app.debtFromPartners')"
                                    :value="currency_format($statistics->partner_debt, currency_get('USD')->id)"
                                    icon="layer-group">
                    </x-cards.widget>
                </a>
            </div>
            <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
                <a href="{{ route('applications.index')}}">
                    <x-cards.widget :title="__('app.menu.applications')"
                                    :value="$statistics->client_count"
                                    icon="layer-group">
                    </x-cards.widget>
                </a>
            </div>
...
@push('scripts')
    ...
    <script>
        $('#stats-table').on('preXhr.dt', function (e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["stats-table"].draw(false);
        }

        $('#client, #operator, #partner, #status, #filter_category_id, #filter_sub_category_id, #project_id, #contract_type_id, #country_id, #verification')
            .on('change keyup', function () {
                if ($('#status').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
                } else if ($('#client').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
                } else if ($('#operator').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
                } else if ($('#partner').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#stats-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();
