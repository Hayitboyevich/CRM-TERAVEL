                ...
                <div class="row">
                    <div class="col-xl-7 col-md-6 mb-4 mb-lg-0">
                                    ...
                                    <div class="d-flex flex-wrap justify-content-between">
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Все лиды</label>
                                            <p class="mb-0 f-18 f-w-500">{{ array_sum(array_values($leadStats)) }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Несортированный</label>
                                            <p class="mb-0 f-18 f-w-500">{{ Arr::get($leadStats, 2) ?? 0 }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">В Процессе</label>
                                            <p class="mb-0 f-18 f-w-500">{{ Arr::get($leadStats, 3) ?? 0 }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Частично / Оплаченный</label>
                                            <p class="mb-0 f-18 f-w-500">{{ (Arr::get($leadStats, 5) ?? 0) . '/'.(Arr::get($leadStats, 6) ?? 0) . '/'.(Arr::get($leadStats, 5) ?? 0)+(Arr::get($leadStats, 6) ?? 0)}}</p>
                                        </span>
                                    </div>
                                    <div class="d-flex flex-wrap justify-content-between">
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Заключен</label>
                                            <p class="mb-0 f-18 f-w-500">{{ Arr::get($leadStats, 7) ?? 0 }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Отменено</label>
                                            <p class="mb-0 f-18 f-w-500">{{ Arr::get($leadStats, 8) ?? 0 }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">Profit</label>
                                            <p class="mb-0 f-18 f-w-500">{{ $profit_employee }}</p>
                                        </span>
                                        <span>
                                            <label class="f-11 text-dark-grey mb-12 text-capitalize"
                                                   for="usr">All sum</label>
                                            <p class="mb-0 f-18 f-w-500">{{ $profit_all }}</p>
                                        </span>
                                    </div>
                                </div>
                            @endif
                        ...
                        <x-cards.data :title="__('modules.client.profileInfo')" class=" mt-4">
                            ...
                            @php
                                $currentyearJoiningDate = Carbon::parse(now(company()->timezone)->year.'-'.$employee->employeeDetail->joining_date->translatedFormat('m-d'));
                            ...
                            <x-cards.data-row :label="__('modules.employees.workAnniversary')"
                                              :value="(!is_null($employee->employeeDetail) && !is_null($employee->employeeDetail->joining_date)) ? (($diffInHoursJoiningDate > -23 && $diffInHoursJoiningDate <= 0) ? __('app.today') : $currentyearJoiningDate->longRelativeToNowDiffForHumans()) : '--'"/>
                            ...
                            @if ($showFullProfile)
                                ...
                                <x-forms.custom-field-show :fields="$fields"
                                                           :model="$employee->employeeDetail"></x-forms.custom-field-show>
                            ...
                            <x-cards.data class="mb-4" :title="__('modules.appreciations.appreciation')">
                                @forelse ($employee->appreciationsGrouped as $item)
                                    <div class="float-left position-relative mb-2" style="width: 50px"
                                         data-toggle="tooltip"
                                         data-original-title="@if(isset($item->award->title)){{  mb_ucwords($item->award->title) }} @endif">
                                                    ...
                                                    <div class="taskEmployeeImg rounded-circle mr-1">
                                                        <a href="{{ route('employees.show', $item->user->id) }}">
                                                            <img data-toggle="tooltip"
                                                                 data-original-title="{{ mb_ucwords($item->user->name) }}"
                                ...
                                @if (in_array('attendance', user_modules()))
                                    <div class="col-xl-6 col-sm-12 mb-4">
                                        <x-cards.widget :title="__('modules.dashboard.lateAttendanceMark')"
                                                        ...
                                                        :info="__('modules.dashboard.thisMonth')"
                                                        icon="map-marker-alt"/>
                                    </div>
                                @endif
