    ...
    <x-table class="table-bordered">
        ...
        @forelse($leadStatus as $key => $status)
            <tr class="row{{ $status->id }}">
                ...
                <td>
                    <x-forms.radio fieldId="client_from_{{ $status->id }}" class="set_default_client_from"
                                   data-status-id="{{ $status->id }}" :fieldLabel="__('app.default')"
                                   fieldName="client_from" fieldValue="{{ $status->id }}"
                                   :checked="($status->client_from == 1) ? 'checked' : ''">
                    </x-forms.radio>
                </td>
