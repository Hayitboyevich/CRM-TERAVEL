...
@section('filter-section')
    ...
    <div class="d-flex filter-box project-header bg-white">
        ...
        <div class="project-menu d-lg-flex" id="mob-client-detail">
            ...
            <x-tab :href="route('clients.show', $client->id).'?tab=application'" ajax="false"
                   :text="__('app.menu.applications')"
                   class="application"/>

{{--            <x-tab :href="route('clients.show', $client->id).'?tab=integration'" ajax="false"--}}
{{--                   :text="__('app.menu.integration')"--}}
{{--                   class="integration"/>--}}

{{--            @if (in_array('projects', user_modules()))--}}
{{--                <x-tab :href="route('clients.show', $client->id).'?tab=projects'" ajax="false"--}}
{{--                       :text="__('app.menu.projects')"--}}
{{--                       class="projects"/>--}}
{{--            @endif--}}

{{--            @if (in_array('invoices', user_modules()))--}}
{{--                <x-tab :href="route('clients.show', $client->id).'?tab=invoices'" ajax="false"--}}
{{--                       :text="__('app.menu.invoices')"--}}
{{--                       class="invoices"/>--}}
{{--            @endif--}}

{{--            @if (in_array('estimates', user_modules()))--}}
{{--                <x-tab :href="route('clients.show', $client->id).'?tab=estimates'" ajax="false"--}}
{{--                       :text="__('app.menu.estimates')"--}}
{{--                       class="estimates"/>--}}
{{--            @endif--}}

{{--            <x-tab :href="route('clients.show', $client->id).'?tab=creditnotes'" ajax="false"--}}
{{--                   :text="__('app.menu.credit-note')"--}}
{{--                   class="creditnotes"/>--}}

            @if (in_array('payments', user_modules()))
                <x-tab :href="route('clients.show', $client->id).'?tab=payments'" ajax="false"
                       :text="__('app.menu.payments')"
                       class="payments"/>
            @endif

            @if ($viewClientContact == 'all' || $viewClientContact == 'added')
                <x-tab :href="route('clients.show', $client->id).'?tab=contacts'" ajax="false"
                       :text="__('app.menu.contacts')"
                       class="contacts"/>
            @endif
            @if ($viewClientDoc == 'all' || ($viewClientDoc == 'added' && $client->clientDetails?->added_by == user()->id) || ($viewClientDoc == 'owned' && $client->clientDetails?->user_id == user()->id) || ($viewClientDoc == 'both' && ($client->clientDetails?->added_by == user()->id || $client->clientDetails?->user_id == user()->id)))
                <x-tab :href="route('clients.show', $client->id).'?tab=documents'" ajax="false"
...
{{--                <x-tab :href="route('clients.show', $client->id).'?tab=tickets'" ajax="false"--}}
{{--                       :text="__('app.menu.tickets')"--}}
{{--                       class="tickets"/>--}}
{{--            @endif--}}
