...
@push('scripts')
    ...
    <script>
        ...
        $('#order-items-table').on('preXhr.dt', function (e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["order-items-table"].draw(false);
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#order-items-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();
        ...
        $(document).ready(function () {
            $('#order-items-table').on('click', 'tbody tr', function () {
                if (!$(event.target).is(':checkbox')) {
                    const id = $(this)[0].id.split('-')[1];
                    var url = '{{ route("services.edit", ":id") }}'.replace(':id', id);
