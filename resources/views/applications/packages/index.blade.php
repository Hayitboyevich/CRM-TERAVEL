...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('app.verify') <i
                            class="fa fa-question-circle" data-toggle="popover" data-placement="top"
                            data-content="@lang('messages.clientFilterVerification')" data-html="true"
                            data-trigger="hover"></i></label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" id="verification" data-container="body"
                                data-size="8">
                            <option value="all">@lang('app.all')</option>
    ...
    <div class="content-wrapper">
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar dd">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                ...
                <x-forms.link-primary :link="route('packages.create')"
...
@push('scripts')
    ...
    <script>
        ...
        const showTable = () => {
            window.LaravelDataTables["package-table"].draw(false);
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#package-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();

            const url = "{{ route('packages.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $(document).ready(function () {
            $('#package-table').on('click', 'tbody tr', function () {
                if (!$(event.target).is(':checkbox')) {
                    console.log('sd');
                    const id = $(this)[0].id.split('-')[1];
                    var url = '{{ route("packages.edit", ":id") }}'.replace(':id', id);
