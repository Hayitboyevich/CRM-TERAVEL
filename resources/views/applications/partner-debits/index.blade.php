...
@push('scripts')
    ...
    <script>
        ...
        const showTable = () => {
            window.LaravelDataTables["debits-table"].draw(false);
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#debits-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();
        ...
        $(document).ready(function () {
            $('#debits-table').on('click', 'tbody tr', function () {
                console.log($(event.target).attr('id'));
                if (!$(event.target).is(':checkbox')) {
                    const id = $(this)[0].id.split('-')[1];
                    var url = '{{ route("partners.items", ":id") }}'.replace(':id', id);
