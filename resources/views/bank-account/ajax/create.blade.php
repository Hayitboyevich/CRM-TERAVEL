                ...
                <div class="row p-20">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label class="f-14 text-dark-grey mb-12 w-100 mt-3"
                                   for="usr">@lang('modules.bankaccount.type')</label>
                            <div class="d-flex">
                                <x-forms.radio fieldId="type-bank" class="type"
                                               :fieldLabel="__('modules.bankaccount.bank')" fieldValue="bank"
                                               fieldName="type" checked="true">
                                </x-forms.radio>
                                <x-forms.radio fieldId="type-cash" class="type"
                                               :fieldLabel="__('modules.bankaccount.cash')" fieldValue="cash"
                                               fieldName="type">
                                </x-forms.radio>
                                <x-forms.radio fieldId="type-card" class="type"
                                               :fieldLabel="__('modules.bankaccount.card')" fieldValue="card"
                                               fieldName="type">
                                </x-forms.radio>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4 accountNumber">
                        <x-forms.text fieldId="account_number" :fieldLabel="__('modules.bankaccount.accountNumber')"
                                      ...
                                      :fieldPlaceholder="__('placeholders.bankaccount.accountNumber')"
                                      fieldRequired="true"></x-forms.text>
                    </div>

                    <div class="col-md-4 accountType">
                        <x-forms.select fieldId="account_type" :fieldLabel="__('modules.bankaccount.accountType')"
                    ...
                    <div class="col-md-4">
                        <x-forms.tel fieldId="contact_number" :fieldLabel="__('modules.bankaccount.contactNumber')"
                                     fieldName="contact_number"
                                     :fieldPlaceholder="__('placeholders.mobile')" fieldRequired="true"></x-forms.tel>
                    </div>

                    <div class="col-md-4">
                        <x-forms.number fieldId="opening_balance" :fieldLabel="__('modules.bankaccount.openingBalance')"
                                        fieldName="opening_balance"
                                        :fieldValue="0"
                                        :fieldPlaceholder="__('placeholders.price')"
                                        fieldRequired="true"></x-forms.number>
                    </div>
    ...
    $(document).ready(function () {
        ...
        $('.type').change(function () {
            ...
            switch (selectedValue) {
                case 'bank': {
                    $("label[for='account_name']").text("Имя владельца счета");
                    $("label[for='account_number']").text("Номер счета");
                    $('.bank_name').removeClass('d-none');
                    $('.accountNumber').removeClass('d-none');
                    $('.bankLogo').removeClass('d-none');
                    $('.accountType').removeClass('d-none');
                }
                    break;
                case 'cash': {
                    $("label[for='account_name']").text("Имя владельца счета");
                    ...
                    $('.bankLogo').addClass('d-none');
                    $('.accountType').addClass('d-none');
                }
                    break;
                case 'card': {
                    $("label[for='account_name']").text("Имя владельца карты");
                    $("label[for='account_number']").text("Номер карты");
                    $('.accountNumber').removeClass('d-none');
                    $('.bank_name').addClass('d-none');
                    $('.bankLogo').addClass('d-none');
                    $('.accountType').addClass('d-none');
