        ...
        <x-cards.data :title="__('modules.bankaccount.recentTransactions')" class="table-hover" padding="false">
            <x-table>
                ...
                @forelse ($recentTransactions as $key => $item)
                    @php
                            $typeClass = ($item->type == 'Cr') ? 'text-success' : (($item->type == 'Dr') ? 'text-danger' : '');
                            $sign = ($item->type == 'Dr') ? '-' : '+';
                            $currencyId = $item->currency_id ? $item->currency_id : $bankaccount->currency_id;
                    @endphp
                    <tr>
                        <td class="pl-20">
                            {{ ($key+1) }}
                        ...
                        <td class="pr-20 text-right">
                            <span class="{{ $typeClass }}">
                                {{ $sign }} {{ currency_format($item->amount, $currencyId) }}
                            </span>
                        </td>
                    </tr>
                @empty
