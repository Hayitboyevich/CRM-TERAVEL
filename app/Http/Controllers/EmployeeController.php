...
class EmployeeController extends AccountBaseController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            ...
            if ($user->id) {
                ...
                $employee->service_rate = $request->service_rate;
    ...
    public function show($id)
    {
        ...
        $statuses = LeadStatus::query()
            ->whereNotIn('type', ['Не Качественный Лид'])
        ...
        $ls = Lead::query()
            ->selectRaw('lead_status.priority, COUNT(*) as count')
            ->join('lead_status', 'lead_status.id', '=', 'leads.status_id')
            ->where('lead_status.company_id', company()->id)
            ->whereIn('agent_id', $this->employee->leadAgent->pluck('id')->toArray());

        $values = $ls->whereIn('status_id', $statuses);
        $sum = $values->sum('value');

        $service_rate = $this->employee->employeeDetail->service_rate ?? 0;
        $profit = $sum * ($service_rate / 100);

        $this->profit_employee = currency_format($profit, company()->currency_id);
        $this->profit_all = currency_format($sum, company()->currency_id);

        $this->leadStats = $ls
            ->orderBy('lead_status.priority')
            ->groupBy('lead_status.priority')
            ->pluck('count', 'lead_status.priority')
            ->toArray();
