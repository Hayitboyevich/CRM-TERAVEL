...
use App\Http\Requests\CustomStorePayment;
...
class PaymentController extends AccountBaseController
{
    ...
    public function customCreate($orderId)
    {
        $this->addPermission = user()->permission('add_payments');
        abort_403(!in_array($this->addPermission, ['all', 'added']));
        ...
        $this->linkPaymentPermission = user()->permission('link_payment_bank_account');
        $this->companyCurrency = Currency::query()->where('id', company()->currency_id)->first();

        $this->bankDetails = BankAccount::query()->where('status', 1)
            ->get();
        ...
        if (request()->ajax()) {
            $html = view('payments.ajax.custom-create', $this->data)->render();
            return Reply::dataOnly(['status' => 'success', 'html' => $html, 'title' => $this->pageTitle]);
        }
        $this->view = 'payments.ajax.custom-create';
        return view('payments.create', $this->data);
    }

    public function customStore(CustomStorePayment $request)
    {
        DB::beginTransaction();
        try {
            $payment = new Payment();
            ...
            $paid = $request->amount * $request->exchange_rate;
            $order->total_paid = ($order->total_paid + round($paid, 2));
            $order->save();
            $lead = Lead::query()->where(['order_id' => $order->id])->first();

            $statues = LeadStatus::query()->where(['company_id' => company()->id])
                ->where(['priority' => 5])
                ->orWhere(['priority' => 6])
                ->get();

            if ($order->total_paid >= $order->total) {
                $lead->status_id = $statues[1]?->id;
                $lead->save();

                $order->status = OrderStatus::STATUS_COMPLETED;
                $order->save();

            } elseif ($order->total_paid < $order->total) {
                $lead->status_id = $statues[0]?->id;
                $lead->save();
            ...
            DB::rollBack();
            return Reply::error($exception->getMessage(), ['redirectUrl' => route('leads.show', $lead->id)]);
    ...
    public function edit($id)
    {
        ...
        $bankAccounts = BankAccount::where('status', 1)->where('company_id', company()->id);
    ...
    public function getCurrency(Request $request)
    {
        ...
        return Reply::dataOnly(['status' => 'success', 'currency' => $currency]);
    }
