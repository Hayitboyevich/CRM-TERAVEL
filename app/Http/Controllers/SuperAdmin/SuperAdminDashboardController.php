...
class SuperAdminDashboardController extends SuperAdminBaseController
{
    ...
    public function index()
    {
        ...
        if (Cache::has('totalCompanies')) {
            // Retrieve total companies count from cache
            $this->totalCompanies = Cache::get('totalCompanies');
        } else {
            // If not cached, execute the query and cache the result
            $this->totalCompanies = Company::count();
            Cache::put('totalCompanies', $this->totalCompanies, 3600);
        }
        ...
        $invoices = StripeInvoice::selectRaw('SUM(amount) as amount,YEAR(pay_date) as year, MONTH(pay_date) as month')->whereNotNull('stripe_invoices.pay_date')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();

        $paypalInvoices = PaypalInvoice::selectRaw('SUM(total) as total,YEAR(paid_on) as year, MONTH(paid_on) as month')->where('paypal_invoices.status', 'paid')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();

        $razorpayInvoice = RazorpayInvoice::selectRaw('SUM(amount) as amount,YEAR(pay_date) as year, MONTH(pay_date) as month')->whereNotNull('razorpay_invoices.pay_date')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();

        $paystackInvoice = PaystackInvoice::selectRaw('SUM(amount) as amount,YEAR(pay_date) as year, MONTH(pay_date) as month')->whereNotNull('paystack_invoices.pay_date')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();

        $mollieInvoice = MollieInvoice::selectRaw('SUM(amount) as amount,YEAR(pay_date) as year, MONTH(pay_date) as month')->whereNotNull('mollie_invoices.pay_date')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();

        $authorizeInvoice = AuthorizationInvoice::selectRaw('SUM(amount) as amount,YEAR(pay_date) as year, MONTH(pay_date) as month')->whereNotNull('authorize_invoices.pay_date')->havingRaw('year = ?', [$year])->groupBy('month')->get()->groupBy('month')->toArray();
