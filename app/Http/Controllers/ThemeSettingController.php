...
class ThemeSettingController extends AccountBaseController
{
    ...
    public function store(UpdateThemeSetting $request)
    {
        ...
        if ($request->check_theme_delete == 'yes') {
            Files::deleteFile($setting->check_theme, 'check_theme');
            $setting->check_theme = null;
        }

        if ($request->hasFile('check_theme')) {
            $setting->check_theme = Files::upload($request->check_theme, 'check_theme');
        }
