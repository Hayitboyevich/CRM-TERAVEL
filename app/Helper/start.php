...
if (!function_exists('push_setting')) {
    ...
    function push_setting()
    {
        if (!session()->has('push_setting')) {
            session(['push_setting' => PushNotificationSetting::first()]);
        }
...
if (!function_exists('currency_get')) {
    ...
    {
        $key = 'cur_' . $currency_code . '_' . company()->id;
        if (Cache::get($key)) {
...
if (!function_exists('currency_get_by_id')) {
    function currency_get_by_id($currency_id)
    {
        $key = 'cur_' . $currency_id . '_' . company()->id;
        if (Cache::get($key)) {
            return Cache::get($key);
        }
        $val = Currency::query()
...
if (!function_exists('sidebar_user_perms')) {
    ...
    function sidebar_user_perms()
    {
        if (!session()->has('sidebar_user_perms')) {
            ...
            $sidebarPermissions = Permission::query()->whereIn('name', $sidebarPermissionsArray)->select('id', 'name')->orderBy('id', 'asc')->get();
...
if (!function_exists('company_usd_rate')) {

    function company_usd_rate()
    {
        if (session()->has('company_usd_rate')) {
            return session('company_usd_rate');
        }

        $company = company();
        if ($company) {
            $rate = Currency::where('currency_code', 'USD')->where('company_id', $company->id)->first()->exchange_rate;
            session(['company_usd_rate' => $rate]);
...
if (!function_exists('company_rub_rate')) {

    function company_rub_rate()
    {
        if (session()->has('company_rub_rate')) {
            return session('company_rub_rate');
        }

        $company = company();
        if ($company) {
            $rate = Currency::where('currency_code', 'RUB')->where('company_id', $company->id)->first()->exchange_rate;
            session(['company_rub_rate' => $rate]);
...
if (!function_exists('company_uzs')) {

    function company_uzs()
    {
        $currency = Currency::where('currency_code', 'UZS')->where('company_id', company()->id)->first();
        return $currency;
    }
}
