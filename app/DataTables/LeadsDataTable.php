...
class LeadsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('lead_note_titles', function ($row) {
            return $row->note_title;
        });
        $datatables->addColumn('lead_note_details', function ($row) {
            return $row->note_detail;
        });
        ...
        $datatables->rawColumns(array_merge(['status', 'action', 'client_name', 'lead_note_titles', 'lead_note_details', 'next_follow_up_date', 'agent_name', 'check', 'mobile'], $customFieldColumns));
    ...
    public function query(Lead $model)
    {
        $lead = $model->with(['leadAgent', 'leadAgent.user', 'category'])
            ->select(
                ...
                DB::raw("GROUP_CONCAT(lead_notes.title) as note_title"),
//                DB::raw("GROUP_CONCAT('(' , lead_notes.title , ' - ' , lead_notes.details , ')') as note_detail"),
                DB::raw("GROUP_CONCAT(lead_notes.title) as note_detail"),
            )
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.notes') => ['data' => 'lead_note_titles', 'name' => 'lead_note_titles', 'exportable' => false, 'title' => __('app.notes')],
            __('app.noteDetails') => ['data' => 'lead_note_details', 'name' => 'lead_note_details', 'visible' => false, 'title' => __('app.notes')],
