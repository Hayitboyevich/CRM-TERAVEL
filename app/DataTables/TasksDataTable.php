...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->rawColumns(array_merge(['short_code', 'board_column', 'action', 'project_name', 'clientName', 'due_date', 'users', 'heading', 'check', 'timeLogged', 'timer', 'start_date'], $customFieldColumns));
    ...
    public function query(Task $model)
    {
        ...
        $model = $model->leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
            ...
            ->leftJoin('mention_users', 'mention_users.project_id', 'tasks.id');
        ...
        $model->leftJoin('users as creator_user', 'creator_user.id', '=', 'tasks.created_by')
            ...
            ->selectRaw(
                'tasks.id, tasks.task_short_code, tasks.added_by, projects.project_name, projects.project_admin, tasks.heading, client.name as clientName, creator_user.name as created_by, creator_user.image as created_image, tasks.board_column_id,
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.menu.tasks') . ' ' => ['data' => 'task', 'name' => 'heading', 'visible' => false, 'title' => __('app.menu.tasks')],            __('modules.tasks.assigned') => ['data' => 'name', 'name' => 'name', 'visible' => false, 'title' => __('modules.tasks.assigned')],
