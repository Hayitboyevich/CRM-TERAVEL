...
class PaymentsDataTable extends BaseDataTable
{
        ...
        return $datatables
            ...
            ->editColumn('order_number', function ($row) {
                if (!is_null($row->order_id) && !is_null($row->order)) {
                    return '<a class="text-darkest-grey" href="' . route('orders.show', $row->order_id) . '">' . ucfirst($row->order->order_number) . '-' . $row->order->application_id . '</a>';
                }

                return '--';
            })
            ->editColumn('status', function ($row) {
                if ((in_array('admin', user_roles()) || in_array('employee', user_roles())) && ($this->editPaymentPermission == 'all' || ($this->editPaymentPermission == 'both' && ($row->added_by == user()->id || $row->client_id == user()->id)) || ($this->editPaymentPermission == 'added' && $row->added_by == user()->id) || ($this->editPaymentPermission == 'owned' && $row->client_id == user()->id))) {

                    $status = '<select class="form-control select-picker payment-status" data-payment-id="' . $row->id . '">';

                    $status .= '<option value="complete" data-content="<i class=\'fa fa-circle mr-2 text-light-green\'></i> ' . __('app.completed') . '"';
                    if ($row->status == 'complete') {
                        $status .= ' selected';
                    }
                    $status .= '>' . __('app.completed') . '</option>';

                    $status .= '<option value="failed" data-content="<i class=\'fa fa-circle mr-2 text-red\'></i> ' . __('app.failed') . '"';
                    if ($row->status == 'failed') {
                        $status .= ' selected';
                    }
                    $status .= '>' . __('app.failed') . '</option>';

                    $status .= '<option value="pending" data-content="<i class=\'fa fa-circle mr-2 text-yellow\'></i> ' . __('app.pending') . '"';
                    if ($row->status == 'pending') {
                        $status .= ' selected';
                    }
                    $status .= '>' . __('app.pending') . '</option>';
            ...
            ->editColumn('type', function ($row) {
                $badgeColor = match ($row->type) {
                    Payment::TYPE_CREDIT => '#28a745', // green
                    Payment::TYPE_DEBIT => '#dc3545', // red
                    default => '#ffc107', // yellow
                };

                return '<span style="color: white; background-color: ' . $badgeColor . '; padding: 3px 6px; border-radius: 5px;">' . ucfirst($row->type) . '</span>';
            })
            ->addIndexColumn()
            ->smart(false)
            ...
            ->rawColumns(['invoice', 'action', 'status', 'client_id', 'client_email', 'project_id', 'invoice_number', 'order_number', 'check', 'type'])
    ...
    public function query()
    {
        ...
        $model = Payment::with(['invoice.client', 'order.client', 'currency:id,currency_symbol,currency_code'])
            ...
            ->select('payments.id', 'payments.company_id', 'payments.project_id', 'payments.currency_id', 'payments.invoice_id', 'payments.amount', 'payments.status', 'payments.paid_on', 'payments.remarks', 'payments.bill', 'payments.added_by', 'payments.order_id', 'payments.gateway', 'payments.transaction_id', 'payments.type');
        ...
        if ($request->type != 'all' && !is_null($request->type)) {
            $model = $model->where('payments.type', '=', $request->type);
        }
        ...
        return $this->setBuilder('payments-table', 2)
            ->parameters([
                ...
                'fnDrawCallback' => 'function( oSettings ) {
                    $("#payments-table .select-picker").selectpicker();
                    $("body").tooltip({
