            ...
            if (!is_null($payment->bank_account_id) && $payment->status == 'complete') {
                if ($payment->isDirty('bank_account_id')) {
                    ...
                    if ($payment->getOriginal('amount') > $payment->amount) {
                        ...
                        $transaction->payment_id = $payment->id;
                        $transaction->invoice_id = $payment->invoice_id;
                        $transaction->type = $payment->type == 'credit' ? 'Cr' : 'Dr';
                    ...
                    $bankAccount = BankAccount::find($payment->bank_account_id);
                    $bankBalance = $bankAccount->bank_balance;
                    if ($payment->type == 'credit' && $payment->paid_for == 'client') {
                        // add payment to account
                        $amount = $payment->amount;
                        $newBalance = $bankBalance + $payment->amount;
                    } elseif ($payment->type == 'debit' && $payment->paid_for == 'client') {
                        // remove payment to account
                        $newBalance = $bankBalance - $payment->amount;
                        $amount = (-1) * $payment->amount;

                    } elseif ($payment->type == 'credit' && $payment->paid_for == 'partner') {
                        // remove payment to account
                        $newBalance = $bankBalance + $payment->amount;
                        $amount = $payment->amount;

                    } elseif ($payment->type == 'debit' && $payment->paid_for == 'partner') {
                        // add payment to account
                        $amount = (-1) * $payment->amount;
                    ...
                    if ($newBalance < 0) {
                        throw new Exception('На вашем банковском счете недостаточно средств');
                    }
                    ...
                    $transaction->transaction_relation = 'payment';
                    $transaction->transaction_related_to = $payment->id;
                    $transaction->title = $payment->type == 'credit' ? 'payment-credited' : 'payment-debited';
                    $transaction->save();
...
//            }
            if ($payment->isDirty('status') && $payment->status == 'complete') {
                $order = $payment->order;
                $amount = $payment->amount;
                if ($payment->currency_id != currency_get('USD')->id) {
                    $amount = $amount / $payment->exchange_rate;
                }
                if (company()->currency->id != currency_get('USD')->id) {
                    $amount = $amount * $payment->exchange_rate;
                }
                if ($payment->type == 'credit' && $payment->paid_for == 'client') {
                    $order->total_paid = ($order->total_paid) + $amount;
                } elseif ($payment->type == 'debit' && $payment->paid_for == 'client') {
                    $order->total_paid = ($order->total_paid) - $amount;
                } elseif ($payment->type == 'credit' && $payment->paid_for == 'partner') {
                    $order->net_price_paid = ($order->net_price_paid) - $amount;
                } elseif ($payment->type == 'debit' && $payment->paid_for == 'partner') {
                    $order->net_price_paid = ($order->net_price_paid) + $amount;
                }
                $order->save();
