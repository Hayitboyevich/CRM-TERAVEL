...
class CompanyObserver
{
    ...
    private function copyFromGlobalSettings($company)
    {
        ...
        $company->check_theme = $globalSetting->check_theme;
    ...
    public function saving(Company $company)
    {
        ...
        // for the case of running company migration before having global_setting table
        if ($company->id === 1 && isWorksuite() && !isRunningInConsoleOrSeeding()) {
            ...
            $global->check_theme = $company->check_theme;
...
//        $currency->saveQuietly();
        ...
        $currency = new Currency();
        $currency->currency_name = 'Sum';
        $currency->currency_symbol = 'UZS';
        $currency->currency_code = 'UZS';
        $currency->exchange_rate = 12500;
        $currency->currency_position = 'left';
        $currency->no_of_decimal = 1;
        $currency->thousand_separator = ',';
        $currency->decimal_separator = '.';
        $currency->company()->associate($company);
        $currency->saveQuietly();
        ...
        $currency = new Currency();
        $currency->currency_name = 'Rubl';
        $currency->currency_symbol = '₽';
        $currency->currency_code = 'RUB';
        $currency->exchange_rate = 92;
        $currency->currency_position = 'left';
        $currency->no_of_decimal = 2;
        $currency->thousand_separator = ',';
        $currency->decimal_separator = '.';
        ...
        $currency = new Currency();
        $currency->currency_name = 'Dirham';
        $currency->currency_symbol = 'DIR';
        $currency->currency_code = 'DRH';
        $currency->exchange_rate = 30000;
        $currency->currency_position = 'left';
        $currency->no_of_decimal = 2;
        $currency->thousand_separator = ',';
        $currency->decimal_separator = '.';
    ...
    public function companyAddress($company)
    {
        $company->companyAddress()->create([
            ...
            'location' => $company->company_name ?? 'Tashkent, Uzbekistan',
            'is_default' => 1,
    ...
    public function leadSources($company)
    {
        ...
        $status = [
            ['type' => 'Клиенты из Target', 'priority' => 1,
                'default' => 1,
                'label_color' => '#020202',
                'company_id' => $company->id],
            ['type' => 'Несортированный',
                'priority' => 2,
                'default' => 0,
                'label_color' => '#FFE700',
                'company_id' => $company->id],
            ['type' => 'В процессе', 'priority' => 3, 'default' => 0, 'label_color' => '#009EFF', 'company_id' => $company->id],
            ['type' => 'Нужно выйти на связь !!!', 'priority' => 4, 'default' => 0, 'label_color' => '#25FF00', 'company_id' => $company->id],

            ['type' => 'Частично оплачен', 'priority' => 5, 'default' => 0, 'label_color' => '#0CFFFF', 'company_id' => $company->id],
            ['type' => 'Оплаченный', 'priority' => 6, 'default' => 0, 'label_color' => '#Оплаченный', 'company_id' => $company->id],
            ['type' => 'Документы выданы', 'priority' => 7, 'default' => 0, 'label_color' => '#16813D', 'company_id' => $company->id],
            ['type' => 'Не качественный ЛИД', 'priority' => 8, 'default' => 0, 'label_color' => '#FD0410', 'company_id' => $company->id],
    ...
    public function kpi($company)
    {
        DB::transaction(function () use ($company) {
            $kpi = new KpiItem();
            $kpi->name = 'LeadCalculateKPIService';
            $kpi->percent = 25;
            $kpi->expected_amount = 10;
            $kpi->company_id = $company->id;
            $kpi->save();

            $kpi = new KpiItem();
            $kpi->name = 'DeadlineMetricKPIService';
            $kpi->percent = 25;
            $kpi->expected_amount = 10;
            $kpi->company_id = $company->id;
            $kpi->save();
            ...
            $kpi = new KpiItem();
            $kpi->name = 'ProfitKPIService';
            $kpi->percent = 25;
            $kpi->expected_amount = 10000;
            $kpi->company_id = $company->id;
            $kpi->save();

            $kpi = new KpiItem();
            $kpi->name = 'RegularCustomerKPIService';
            $kpi->percent = 25;
            $kpi->expected_amount = 10;
            $kpi->company_id = $company->id;
            $kpi->save();
    ...
    public function productSetting($company)
    {
        foreach (['visa' => 'Виза', 'insurance' => 'Страхование',
                     'hotel' => 'Гостиница', 'transfer' => 'Трансфер',
                     'airticket' => 'Авиабилет',
                     'tourpackage' => 'Тур пакет',
                     'tourservice' => 'Услуги'
                 ] as $key => $item) {
            $product = new Product();
            $product->name = $key;
