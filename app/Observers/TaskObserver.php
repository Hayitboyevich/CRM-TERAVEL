...
class TaskObserver
{
    ...
    public function created(Task $task)
    {
        if (!isRunningInConsoleOrSeeding()) {
            $mention_ids = [];
            $mention_description_members = null;
            $unmention_ids = null;
            $unmention_description_member = null;
            ...
            if (request()->has('project_id') && request()->project_id != 'all' && request()->project_id != '') {
                if ((request()->mention_user_id) != null || request()->mention_user_id != '' || $mention_ids != null && $mention_ids != '') {

                        event(new TaskEvent($task, $mention_description_members, 'TaskMention'));
                    ...
                    if (request()->user_id != null || request()->user_id != '' || request()->has('user_id')) {
                        ...
                        if ($unmention_ids != null && $unmention_ids != '') {
                            ...
                            event(new TaskEvent($task, $unmention_description_member, 'NewTask'));
    ...
    public function updating(Task $task)
    {
        ...
        $mentioned_user = MentionUser::where('task_id', $task->id)->pluck('user_id');
        ...
        if ($request_mention_ids != null) {
            ...
            if (!empty($newMention)) {
                ...
                event(new TaskEvent($task, $new_mention_members, 'TaskMention'));
